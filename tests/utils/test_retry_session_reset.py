    ...
    def test_non_5xx_errors_reset_counter(self, reset_config):
        ...
        def mixed_errors():
            ...
            if call_count == 1:
                ...
                raise HTTPError("Too Many Requests", status_code=429)   # Second error: non-5XX error (should reset counter)
            elif call_count == 3:
                raise HTTPError("Server Error", status_code=500)  # Third error: back to 500 (counter was reset)
            else:
    ...
    def test_session_reset_delay_optimization(self, reset_config):
        ...
        def request_with_reset():
            ...
            if call_count <= config.session_reset_on_5xx_threshold and session_reset_count == 0:
                raise HTTPError("Server Error", status_code=500)
            elif call_count == config.session_reset_on_5xx_threshold + 1 and session_reset_count == 1:
    ...
    def test_different_5xx_errors_count_together(self, reset_config):
        ...
        def mixed_5xx_errors():
            ...
            if call_count == 1:
                ...
                raise HTTPError("Service Unavailable", status_code=503)  # Should trigger session reset after this
            elif call_count == 3:
                # After session reset, succeed
                return unittest.mock.Mock(status_code=200)
            else:
    ...
    def test_all_5xx_error_codes_tracked(self, reset_config):
        ...
        def various_5xx_errors():
            ...
            if call_count == 1:
                ...
                raise HTTPError("Gateway Timeout", status_code=504)  # Should trigger session reset after this
            elif call_count == 3:
                # After session reset, succeed
                return unittest.mock.Mock(status_code=200)
            else:
