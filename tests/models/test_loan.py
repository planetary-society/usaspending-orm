...
class TestLoanSpecificProperties:
    ...
    def test_fain_property(self, mock_usa_client):
        """Test FAIN property."""
        data = {"generated_unique_award_id": "ASST_NON_LOAN_123", "fain": "07123456789"}
        loan = Loan(data, mock_usa_client)
    ...
    def test_uri_property(self, mock_usa_client):
        """Test URI property."""
        data = {"generated_unique_award_id": "ASST_NON_LOAN_123", "uri": "LOAN123XYZ"}
        loan = Loan(data, mock_usa_client)
    ...
    def test_cfda_info_search_results(self, mock_usa_client):
        ...
        cfda_data = [
            {"cfda_number": "59.008", "cfda_program_title": "Disaster Assistance Loans"}
        ]
        data = {
            "generated_unique_award_id": "ASST_NON_LOAN_123",
...
class TestLoanWithRealFixtureData:
    ...
    def test_loan_fixture_properties(self, mock_usa_client, loan_fixture_data):
        ...
        assert (
            loan.cfda_info[0]["cfda_number"]
            == loan_fixture_data["cfda_info"][0]["cfda_number"]
        )
        assert (
            loan.cfda_info[0]["cfda_title"]
            == loan_fixture_data["cfda_info"][0]["cfda_title"]
        )
