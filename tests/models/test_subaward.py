...
class TestSubAwardModel:
    ...
    def test_subaward_parent_award(self, subaward_data, mock_usa_client):
        ...
        if subaward_data.get("prime_award_generated_internal_id"):
            ...
            assert parent.raw["generated_unique_award_id"] == subaward_data["prime_award_generated_internal_id"]
        else:
    ...
    def test_subaward_helper_properties(self, subaward_data, mock_usa_client):
        ...
        from usaspending.utils.formatter import contracts_titlecase, smart_sentence_case, to_date
        
        subaward = SubAward(subaward_data, mock_usa_client)
        
        # Test helper properties map to main properties correctly
        expected_name = contracts_titlecase(subaward_data["Sub-Awardee Name"]) if subaward_data.get("Sub-Awardee Name") else None
        ...
        expected_desc = smart_sentence_case(subaward_data["Sub-Award Description"]) if subaward_data.get("Sub-Award Description") else None
        assert subaward.description == expected_desc
        assert subaward.description == subaward.sub_award_description
        
        expected_date = to_date(subaward_data["Sub-Award Date"]) if subaward_data.get("Sub-Award Date") else None
