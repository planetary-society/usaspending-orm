...
class TestAwardGenericBehaviors:
    ...
    def test_award_identifier_delegates_to_the_parser(self, mock_usa_client):
        """award_identifier exposes the parsed identifier, or "" when there is none.

        The parsing rules themselves are covered in test_award_identifier.py.
        """
        award = Award(
            {"generated_unique_award_id": "CONT_AWD_80GSFC18C0008_8000_-NONE-_-NONE-"},
            mock_usa_client,
        )
        assert award.award_identifier == "80GSFC18C0008"

        # Unparseable IDs surface as an empty string, not None
        placeholder = Award(
            {"generated_unique_award_id": "CONT_AWD_-NONE-_8000_-NONE-_-NONE-"},
            mock_usa_client,
        )
        assert placeholder.award_identifier == ""

        assert Award({"description": "Test Award"}, mock_usa_client).award_identifier == ""
