...
class TestContractSearchResultsAssignment:
    ...
    def contract_fixture_data(self):
        ...
        fixture_path = (
            Path(__file__).parent.parent / "fixtures" / "awards" / "contract.json"
        )
        with open(fixture_path) as f:
            return json.load(f)

    def test_lazy_loading_properties_trigger_fetch(
        self, mock_usa_client, search_results_data, contract_fixture_data
    ):
        ...
        assert len(executive_details["officers"]) == len(
            contract_fixture_data["executive_details"]["officers"]
        )
        award._fetch_details.assert_not_called()  # Should not fetch again
...
class TestGrantsSearchResultsAssignment:
    ...
    def test_all_grants_no_fetch_on_basic_properties(
        self, grant_award_objects, award_index
    ):
    ...
    def grant_fixture_data(self):
        ...
        fixture_path = (
            Path(__file__).parent.parent / "fixtures" / "awards" / "grant.json"
        )
        with open(fixture_path) as f:
            return json.load(f)

    def test_lazy_loading_grant_properties_trigger_fetch(
        self, mock_usa_client, grants_search_results_data, grant_fixture_data
    ):
...
class TestIDVSearchResultsAssignment:
    ...
    @pytest.mark.parametrize("award_index", [0, 1, 2])
    def test_all_idvs_no_fetch_on_basic_properties(
        self, idv_award_objects, award_index
    ):
    ...
    def test_lazy_loading_idv_properties_trigger_fetch(
        self, mock_usa_client, idv_search_results_data, idv_fixture_data
    ):
        """Test that IDV properties not in search results trigger _fetch_details() and return correct values."""
        # Create Award from search results (minimal data)
        search_result = idv_search_results_data[
            1
        ]  # Second IDV (matches detail fixture)
        award = Award(search_result, mock_usa_client)
