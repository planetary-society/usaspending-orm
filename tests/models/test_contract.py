...
class TestContractSpecificProperties:
    ...
    def test_piid_property(self, mock_usa_client):
        """Test PIID property."""
        data = {"generated_unique_award_id": "CONT_AWD_123", "piid": "80GSFC18C0008"}
        contract = Contract(data, mock_usa_client)
    ...
    def test_naics_properties(self, mock_usa_client):
        ...
        assert (
            contract.naics_description == "RESEARCH AND DEVELOPMENT IN NANOTECHNOLOGY"
        )
    ...
    def test_psc_properties(self, mock_usa_client):
        ...
        data = {
            "generated_unique_award_id": "CONT_AWD_123",
            "psc": {"code": "AR11", "description": "SPACE R&D SERVICES"},
        }
        contract = Contract(data, mock_usa_client)
...
class TestContractCachedProperties:
    ...
    def test_psc_hierarchy(self, mock_usa_client):
        ...
        psc_data = {
            "toptier_code": {"code": "A", "description": "RESEARCH AND DEVELOPMENT"},
            "base_code": {"code": "AR11", "description": "SPACE R&D SERVICES"},
        }
        data = {"generated_unique_award_id": "CONT_AWD_123", "psc_hierarchy": psc_data}
        contract = Contract(data, mock_usa_client)
...
class TestContractWithRealFixtureData:
    ...
    def test_contract_fixture_properties(self, mock_usa_client, contract_fixture_data):
        ...
        assert (
            ...
            == contract_fixture_data["base_exercised_options"]
        )
        assert (
            contract.base_and_all_options
            == contract_fixture_data["base_and_all_options"]
        )

        # Test classification data
        assert contract.latest_transaction_contract_data is not None
        assert (
            contract.latest_transaction_contract_data["solicitation_identifier"]
            == contract_fixture_data["latest_transaction_contract_data"][
        ...
        assert (
            contract.psc_hierarchy["toptier_code"]["code"]
            == contract_fixture_data["psc_hierarchy"]["toptier_code"]["code"]
        )
        assert (
            contract.naics_hierarchy["base_code"]["code"]
            == contract_fixture_data["naics_hierarchy"]["base_code"]["code"]
        )
